...
class Tracker:
    ...
    def detect_frames(self, frames, model, conf=0.1):
        batch_size = 20
        detections = []
        for i in range(0, len(frames), batch_size):
            detections_batch = model.predict(frames[i:i + batch_size], conf=conf)
    ...
    def get_object_tracks(self, frames, read_from_stub=False, stub_path=None):
        ...
        player_detections = self.detect_frames(frames, model=self.model, conf=0.10)
        ball_detections = self.detect_frames(frames, model=self.ball_model, conf=0.10)
        ...
        for frame_num, detection in enumerate(player_detections):
            ...
            player_class_id = cls_names_inv.get("player")
            referee_class_id = cls_names_inv.get("referee")
            goalkeeper_class_id = cls_names_inv.get("goalkeeper")

            for object_ind, class_id in enumerate(detection_supervision.class_id):
                if (
                    goalkeeper_class_id is not None
                    and player_class_id is not None
                    and class_id == goalkeeper_class_id
                ):
            ...
            ball_class_id = ball_cls_names_inv.get("ball")
            if ball_class_id is None and len(ball_cls_names_inv) == 1:
                ball_class_id = next(iter(ball_cls_names_inv.values()))
